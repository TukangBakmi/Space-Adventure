...
public class UnknownAlien extends Enemy
{
    //untuk animasi
    ...
    //untuk atribut
    private int hp = 500;
    public static int attack = 400;
    
    public void addedToWorld(World Stage1)
    {
        for(int i=1; i<images.length; i++){
            images[i] = new GreenfootImage("unknownAlien ("+i+").png");
        }
    ...
    private void getHit()
    {
        ...
        if(hp <= 0)
        {
            ...
            getWorld().addObject(new Coin(), this.getX()-40,this.getY()-20);
            //score
            ScoreIcon.myScore += 80;
            //explode
            getWorld().addObject(new UnknownAlienExplode(), getX(), getY());
            //backsound
            GreenfootSound hitEnemy = new GreenfootSound("hitEnemy2.wav");
            hitEnemy.setVolume(90);
            hitEnemy.play();
