...
public class SmallRobot extends Enemy
{
    //untuk animasi
    private GreenfootImage[] images = new GreenfootImage[49];
    private GreenfootImage[] mirroredImages = new GreenfootImage[49];
    private int pause = 2;
    ...
    //untuk pergerakan
    int dx;
    int xSpeed = 4; //kecepatan bergerak horizontal
    //untuk atribut
    private int hp = 180;
    ...
    public void addedToWorld(World Stage1)
    {
        for(int i=1; i<images.length; i++){
            images[i] = new GreenfootImage("smallRobot ("+i+").png");
        }
        for(int j = 1; j<images.length; j++){
            GreenfootImage mirroredImage = new GreenfootImage("smallRobot ("+j+").png");
    ...
    private void getHit()
    {
        ...
        if(hp <= 0)
        {
            ...
            //explode
            getWorld().addObject(new SmallRobotExplode(), getX(), getY());
            //backsound
            GreenfootSound hitEnemy = new GreenfootSound("hitEnemy1.wav");
            hitEnemy.setVolume(90);
            hitEnemy.play();
